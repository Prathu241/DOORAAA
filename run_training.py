...
Training from scratch (--restore_checkpoint 0)
Output: quantized streaming TFLite
"""
...
def main():
    ...
    yaml_config = project_root / 'training_parameters_DORA.yaml'
    ...
    if not yaml_config.exists():
        print(f"  [ERROR] Config not found: {yaml_config}")
        return 1
    print(f"  [OK] Config found: {yaml_config}")
    
    if not mww_dir.exists():
        print(f"  [ERROR] microWakeWord dir not found: {mww_dir}")
        return 1
    print(f"  [OK] microWakeWord found: {mww_dir}")
